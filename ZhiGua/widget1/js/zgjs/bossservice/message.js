...
function _init_listview_message_manage() {
	...
	ajaxSend("WechatBossMessage_messageList", data, "GET", "JSON", true, "_listview_message_manage");
}
...
function _listview_message_manage(data) {
	...
	for (var i = 0; i < messageList.length; i++) {
		...
		} else if (messageList[i].msgType == 6) {// 6供应商消息（消息中含链接）
			_class_names = "message-list-icon icon icon-news-on bg6";
		} else if (messageList[i].msgType == 7 || messageList[i].msgType == 8) {// 7用户产品到期提醒,8产品续费提醒
		...
		var c_pEle = $("<p class='co-back sm row1 v1e5'>" + messageList[i].content.substring(0, 50) + "</p>");
...
function _message_details() {
	...
	ajaxSend("WechatBossMessage_readedMessage", data, "GET", "JSON", true, null);
...
function delete_message(index) {
	...
	ajaxSend("WechatBossMessage_deleteMessage", data, "GET", "JSON", true, "_init_listview_message_manage");
}

function get_noRead_message_count() {
	ajaxSend("WechatBossMessage_getNoReadMessageCount", null, "GET", "JSON", true, "_init_noReadMessageCount", "_error");
}
...
function _init_noReadMessageCount(data) {
	if (data.resultCode == "230105") {
		window.wxc.xcConfirm("账号其它地方登录！", "info", null);
//		alert("账号其它地方登录！");
		window.clearInterval(_check_boss_nrmc);
		_check_boss_nrmc = undefined;
