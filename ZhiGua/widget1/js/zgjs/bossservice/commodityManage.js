...
function add_bar_code() {
	...
	if (validator_varLength("条形码", m_add_bar_code, 20)) {
		...
		data = ajaxSend("WechatBossCommodity_isExist", data, "GET", "JSON", false, null);
...
function add_commodity() {
	...
	ajaxSend("WechatBossCommodity_addCommodity", data, "POST", "JSON", true, "_add_commodity_callback");
	close_win();
}
