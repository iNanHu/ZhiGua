...
function _order_list(data) {
	...
	for (var i = 0; i < morderList.length; i++) {
		...
		if (morderList[i].orderStatus == 3) {
			h5Ele = $("<h5 class='v2e5 co-read co-error'>订单号：<span>" + morderList[i].orderCode + "[冲账]</span></h5>");
		}
...
function _init_m_order_details(data) {
	...
	if (morderList[custOrderIndex].orderStatus != 3 && morderList[custOrderIndex].orderStatus != 4) {
		var cancelEle = $("<span class='ar w20'><div class='mo-btn mo-s' data-co='error' onclick='m_cancelOrder()'>退单</div></span>");
		$('#det_m_bill_cust_order').append(cancelEle);
	}
...
function _printAgain_callback(data) {
	if ("130208" == data.resultCode) {
		...
		window.wxc.xcConfirm('打印订单成功！', "info", null);
		// alert('打印订单成功！');
	}
}

function m_cancelOrder() {
	var option = {
		title : "提示",
		btn : parseInt("0011", 2),
		onOk : function() {
			var ordercode = $('#det_m_bill_cust_code').html();
			var data = {
				"order.orderCode" : ordercode
			};
			ajaxSend("WechatEmpBilling_cancelOrder", data, "GET", "JSON", true, "_m_cancelOrder_callback");
		}
	}
	window.wxc.xcConfirm('您确定要退单吗？', "custom", option);
}

function _m_cancelOrder_callback(data) {
