...
function _commodity_filter() {
	ajaxSend("WechatEmpCommodity_supplierSelect", null, "GET", "JSON", true, "_filter_listview_supplier_name");
}
...
function _init_listview_commodity_manage() {
	...
	ajaxSend("WechatEmpCommodity_commodityList", data, "GET", "JSON", true, "_listview_commodity_manage");
}
...
function _add_commodity() {
	ajaxSend("WechatEmpCommodity_createBarCode", null, "GET", "JSON", true, "init_add_bar_code");
}
...
function add_commodity_photo(dataURL) {
	...
	data = ajaxSend("WechatEmpCommodity_uploadCommodityImage", data, "POST", "JSON", false, null);
...
function add_bar_code() {
	...
	if (validator_varLength("条形码", m_add_bar_code, 20)) {
		var data = {
			"commodity.barCode" : m_add_bar_code
		};
		data = ajaxSend("WechatEmpCommodity_isExist", data, "GET", "JSON", false, null);
		if (data.resultCode == "020401") {
			window.wxc.xcConfirm("商品已存在！", "info", null);
			return false;
		}
...
function add_commodity_name() {
	...
	if (validator_varLength("商品名称", m_add_commodity_name, 16)) {
		var data = {
			"commodity.commodityName" : m_add_commodity_name
		};
		data = ajaxSend("WechatEmpCommodity_isExist", data, "GET", "JSON", false, null);
		if (data.resultCode == "020401") {
			window.wxc.xcConfirm("商品已存在！", "info", null);
...
function _add_supplier_name() {
	ajaxSend("WechatEmpCommodity_supplierSelect", null, "GET", "JSON", true, "_add_listview_supplier_name");
}
...
function _add_commodity_category_name() {
	ajaxSend("WechatEmpCommodity_commodityCategorySelect", null, "GET", "JSON", true, "_add_listview_commodity_category_name");
}
...
function _add_unit_name() {
	ajaxSend("WechatEmpCommodity_commodityUnitSelect", null, "GET", "JSON", true, "_add_listview_unit_name");
}
...
function add_commodity() {
	...
	ajaxSend("WechatEmpCommodity_addCommodity", data, "POST", "JSON", true, "_add_commodity_callback");
	close_win();
}

function _add_commodity_callback(data) {
	if (data.resultCode == "020101") {
		_init_listview_commodity_manage();
		window.wxc.xcConfirm("商品添加成功！", "info", null);
	} else {
		window.wxc.xcConfirm("商品添加失败！", "info", null);
	}
}
...
function update_commodity() {
	...
	ajaxSend("WechatEmpCommodity_updateCommodity", data, "POST", "JSON", true, "update_commodity_callback");
	close_win();
}
...
function update_commodity_callback(data) {
	if (data.resultCode == "020201") {
		...
		_commodity_details();
		_init_listview_commodity_manage();
		window.wxc.xcConfirm("商品更新成功！", "info", null);
	} else {
		window.wxc.xcConfirm("商品更新失败！", "info", null);
	}
}
// 商品编辑 end
// 商品删除 start
function delete_commodity(index) {
	...
	ajaxSend("WechatEmpCommodity_deleteCommodity", data, "POST", "JSON", true, "_init_listview_commodity_manage");
}
// 商品删除 end
