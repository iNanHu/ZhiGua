...
function oPopup(popup) {
	if(popup){
		var title = popup[0];
		var mbUrl = popup[1];
		var w = popup[2];
		var h = popup[3];
		var Callback = popup[4];
		var CallbackData = popup[5];
		
		$('#floater').addClass('on').find('.mo-popup').addClass('on').css({
			'width': w,
			'height': h
		}).find('.popup_title h5').text(title).parents('.mo-popup').find('.popup_content').load(mbUrl, function () {
			console.log('绑定数据,然后回调ue.oPopupEnd');
			if(Callback){
				var fun = eval(Callback);
				if(CallbackData){
					new fun(CallbackData);
				}else{
					new fun();
				}
			}
...
function xPopup() {
    console.log('关闭弹出框');
    $('#floater').removeClass('on').find('.mo-popup').removeClass('on');
}