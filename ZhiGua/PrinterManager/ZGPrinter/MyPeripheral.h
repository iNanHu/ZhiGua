...
#define STATE_NOPAPER_UNMASK 0x01
#define STATE_OVERHEAT_UNMASK 0x02
#define STATE_BATTERYLOW_UNMASK 0x04
#define STATE_PRINTING_UNMASK 0x08
#define STATE_COVEROPEN_UNMASK 0x10
...
typedef struct tagSendData
{
    ...
    int length;
    int offset;
}__attribute__((packed)) SEND_PARAM;
...
@property(assign,readwrite) CONNECTION_PARAMETER_FORMAT connectionParameter;
@property(assign) CONNECTION_PARAMETER_FORMAT backupConnectionParameter;
@property(readwrite) SEND_PARAM sendParam;
...
-(int)read:(Byte*)data length:(int)length timeout:(int)timeOut;
